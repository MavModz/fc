<template>
  <div class="bg-white px-[20px] py-[20px] flex justify-between gap-2 flex-wrap rounded-[4px] cursor-pointer">
    ...
    <span @click="category_update" class="bg-linear-btn hover-effect"> <i class="bi bi-plus"
    ...
    <div v-for="(item, index) in countriesVisaCategories" :key="index + item._id"
      ...
      <div class="">
        <div class="flex gap-2">
          ...
          <span @click="$router.push(`/admin/Categories/${item.service}`)" class="text-[20px] text-[#0D062D] font-bold cursor-pointer">
            {{ item.service }}
          </span>
      ...
      <div class="flex justify-between flex-wrap items-center gap-2">
        ...
        <span class="text-[#282823] font-[400] flex items-center gap-1 px-[10px] py-[5px] rounded-lg"
          ...
          @click="setLocation(item.service)">
        ...
        <div v-if="item.countriesInfo?.length > 0" class="avatars">
          <a v-for="(country, idx) in item.countriesInfo.slice(0, 3)" :key="idx" href="#" class="avatars__item">
  ...
  <div class="offcanvas offcanvas-end offcanv-strst-filter max-w-[560px!important] w-[100%!important]" tabindex="-1"
    ...
    <div class="modal-body-offcan-trst">
      ...
      <div class="relative mt-[40px]">
        ...
        <div class="flex flex-col gap-3 px-[30px] h-[550px] overflow-auto">
...
export default {
  ...
  methods: {
    ...
    async category_update() {
      ...
      await adminPatch(`/company-update/${this.company_id}`, payload).then((response) => {
    ...
    async handleCheck(item) {
      item.flag = !item.flag;
      await adminGet(`/company-all-services/${item._id}/${item.flag}`);
    },
    ...
    async updateServiceCountries() {
      ...
      try {
        ...
        const response = await adminPut(`/admin-company-update-countries/${this.location}`, payload);
    ...
    async fetchAllServices() {
      try {
        const response = await adminGet('/company-all-services');
    ...
    async fetchAdminCountries(category_name) {
      try {
        const [adminCountriesResponse] = await Promise.all([
          adminGet(`/admin-company-country-providing-service/${category_name}`)
        ]);
        if (

		...
		<div class="webhook facebook-meta">
			...
			<Panel v-if="facebookdata" toggleable :collapsed="true">
				...
				<DataTable v-if="currentTable == 'formpage'" :value="allForms" sortMode="multiple"
					...
					<Column header="Connect" style="width: 15%">
						<template #body="{ data }">
					...
					<Column header="Connect" style="width: 25%">
						<template #body="{ data }">
							<Button v-if="findFormSubscription(data.id)" class="btn btn-primary" label="Connected" :disabled="true"/>
							<Button v-else="findFormSubscription(data.id)" class="btn btn-primary" label="Connect Pipeline"
								@click="tempdata = data; importPipelineModal = true" />
						</template>
					</Column>
...
export default {
	...
	methods: {
		...
		async subscribeToLeadgen(data, campaign_id = null) {
			await adminPost(`/fb-subscribe/${this.facebookdata._id}`, { form_id: data.id, campaign_id: campaign_id })
			try {
				const response = await fetch(

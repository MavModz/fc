      ...
      <div class="tourist-basic-wrapper">
        <div class="tourist-basic-1st">
          ...
          <div class="tourist-bacs-1stbx">
            ...
            <div class="tourist-bsc-dic-wd">
              ...
              <FormKit type="text" validation="required" v-model="formData.package_name" name="package_name"
                placeholder="-----------" class="tourist-basc-inpt1st" validation-label="Package Name"/>
            </div>
          </div>
            ...
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="">
                ...
                <FormKit validation="required" v-model="selectedcountry" type="select" name="country"
                  :options="allCountry" placeholder="Select a Country" @change="getState()" validation-label="Country selection"/>
              </div>
            </div>
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="input-field">
                <label for="field-name">PIN code</label>
                <FormKit validation="required" name="pincode" type="text" placeholder="Enter PIN code" validation-label="PIN"/>
              </div>
            </div>
          </div>
          ...
          <div class="row mt-2">
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="mb-3">
                ...
                <FormKit validation="required" v-model="selectedstate" type="select" name="state" :options="allState" placeholder="Select a State"
                  @change="getCity()" />
              </div>
            </div>
            <div class="col-sm-6 mb-3 mb-sm-0">
              <div class="mb-3">
                <div class="mb-3">
                  ...
                  <FormKit validation="required" type="select" name="city" :options="allCity" placeholder="Select a City" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="input-field">
            ...
            <FormKit validation="required" name="address" type="text" placeholder="Destination Address" validation-label="Destination"/>
          </div>

          <div class="input-field">
            <label for="field-name">Description</label>
            <FormKit validation="required" name="overview" as="textarea" placeholder="Add description here"  validation-label="Description"/>
          </div>

          <div class="input-field">
            <label for="field-name">Price</label>
            <FormKit validation="required" name="price" type="number" placeholder="Enter price" validation-label="Price"/>
          </div>

          <div class="tourist-bacs-2stbx">
            <div class="tourist-bsc-dic-wd">
              <label class="tourist-basc-lble">Departure Date</label><br />
              <FormKit type="datetime-local" validation="required|date_after" v-model="formData.departure"
                name="departure" @change="addDate()" class="tourist-basc-inpt1st" validation-label="Departure date"/>
            </div>
            <div class="tourist-bsc-dic-wd">
              <span class="tourist-bsc-dic-checkbx">
                <label class="tourist-basc-lble">Return Date</label>
              </span>
              <FormKit type="datetime-local" validation="required|date_after" v-model="formData.return"
                @change="addDate()" name="return" class="tourist-basc-inpt1st" validation-label="Return date"/>
            </div>
          </div>
              ...
              <div class="tourist-back-addimg-brdr">
                <div v-if="cover_image" class="upload-image-after">
                  ...
                  <label data-kt-image-input-action="change">
                    ...
                    <FormKit validation="required" name="cover_image" type="text" v-model="cover_image" id="icon" validation-label="Cover Image"
                      hidden />
                  </label>
                  <!-- </div> -->
                </div>
...
export default {
  ...
  async mounted() {
    ...
    this.allCountry = (await adminGet(`/service-country/${this.sid}`)).data.data.map(i => ({ label: i, value: i }));
  },
  methods: {
    initData(){
    },
    async submitForm(v) {
      ...
      if (dayDifference < 0) {
        ...
        for (let i in image) {
          v.tourist.images[i] = image[i]
        }
        v.service_id = this.sid;
        v.service_name = 'Tourist Visa';
        v.type = 'tourist';
        ;
        await adminPost(`/add-listing`, v).then((response) => {
          this.loading = false;
          this.$router.push({
            path: '/admin/ViewAllService/TouristList',
            query: { service: v.service_name },
          });
        });
      }
      this.loading = false;
    ...
    async icon_uploadImage() {
      this.cover_image =this.icon_croppedDataUrl

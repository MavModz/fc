    ...
    <FormKit type="form" @submit="submitForm" :value="data">
      <div class="tourist-basic-wrapper">
        <div class="tourist-basic-1st">
          ...
          <div class="tourist-cont-uploadim">
            <div class="tourist-minwidth-qsr">
              ...
              <FormKit type="radio" validation="required"
                :options="{ 'true': 'Yes, we provide it', 'false': 'No, it’s not included' }"
        ...
        <div class="joblis-lsr-nextbtn">
          <div></div>
          <Button type="submit" class="jobls-btnnxt" label="Update Listing" :loading="loading" />
        </div>
...
export default {
  ...
  async setup() {
    ...
    let data = (await adminGet(`/home-listing-details/${route.params.id}`)).data.data;
    const { tourist, ...rest } = data;
    const mergedData = { ...rest, ...tourist };
    data = mergedData
    delete data.tourist
  ...
  async mounted() {
    ...
    this.allCountry = (await adminGet('/countries')).data
    await this.getState()
    await this.getCity()
    this.addDate()
    this.$refs.allimages.all_images = this.data.images
    this.formData.questions = this.data.questions.toString()
  ...
  methods: {
    async submitForm(v) {
      ...
      if (dayDifference < 0) {
        ...
        for (let i in image) {
          v.tourist.images[i] = await image[i]
        }
        v.service_id = "Tourist Visa"
        v.service_name = "Tourist Visa"
        v.type = "tourist"
        delete v._id
        adminPut(`/company-listing/${this.$route.params.id}`, v).then((response) => {
          this.loading = false
          this.$router.push({
    ...
    addDate() {
      if (this.formData.departure && this.formData.return) {
        ...
        let x = this.calculateNightsAndDays(this.formData.departure, this.formData.return)
        this.night = x.night
        this.days = x.days
        ...
        if (dayDifference < 0) {
          ...
          for (let i = 0; i < dayDifference; i++) {
            ...
            this.formData.days.push({ transportation_field: {}, transfer_field: {}, destination_field: {}, accommodation_field: {}, activities_field: {}, custom_field: {}, })
          }
          this.allForm = true
        }
      }

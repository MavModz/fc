<template>
  ...
  <div class="course-buttons-container">
    ...
    <div class="add-followup-button">
      <button class="proceed-button1" @click="addFollowupodal = true">+ Add Followup</button>
    </div>
  </div>
...
export default {
  ...
  async mounted() {
    activateSubmenu('l1', "Edit Lead Details",true);
  ...
  methods: {
    ...
    async init() {
      await adminGet(`/leads/${this.id}`).then(async (r) => {
    ...
    async AddLeads() {
      this.loading = true
      await adminPut(`/update-leads-manualy/${this.$route.params.id}`, this.lead).then(async (r) => {
        this.loading = false
        this.$router.go(-1);
      }).catch((e) => { })
      this.loading = false

...
export default {
  ...
  methods: {
    async init() {
      ...
      this.fieldsData.forEach(mainItem => {
        const filledItem = this.lead.other_fields.find(filledItem => filledItem.field_id === mainItem.field_id);
        if (filledItem) {
          mainItem.field_value = filledItem.field_value;
        }
      });
      this.lead.other_fields = this.fieldsData
    },

          ...
          <template v-if="paginatedFilteredJobs.length">
            <div class="tourist-list">
              <JobsCard v-for="(jobPackage, index) in paginatedFilteredJobs" :key="index"
                ...
                :priceValue="jobPackage.listing_details?.work?.salary_range?.end" :popularCount="jobPackage.popular_count"
                :trendingCount="jobPackage.trending_count" :newCount="jobPackage.new_count"
                :badgeType="getBadgeType(jobPackage)" :viewPackage="() => navigateToPackage(jobPackage)" />
            </div>
          </template>
...
import { fetchCompanies, fetchCompnayData, fetchEvents, whishlistFunction, fetchCategories } from '~/utils/libs/services/api';
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      filterVisible: false,
      filteredBySearch: [], // New property to store search filtered results
    };
  },
  async setup() {
    ...
    searchQuery: {
      handler(newQuery) {
        if (newQuery.trim()) {
          ...
          // Reset to original filtered results when search is cleared
  ...
  methods: {
    ...
    // Add new method to filter by search query
    filterBySearchQuery() {
      ...
      if (this.activeButton === 'business-listing') {
        if (this.category === 'Tourist Visa') {
          this.FilteredPackages = this.packages.filter(pkg => 
            pkg.listing_details?.tourist?.package_name?.toLowerCase().includes(query) ||
            pkg.listing_details?.company_name?.toLowerCase().includes(query)
          );
        } else if (this.category === 'Work Visa') {
          this.FilteredJobs = this.jobs.filter(job => 
            job.listing_details?.work?.title?.toLowerCase().includes(query) ||
            job.listing_details?.company_name?.toLowerCase().includes(query)
          );
        } else {
          this.FilteredCompanies = this.companies.filter(company => 
        ...
        this.FilteredOffers = this.DealOffers.filter(offer => 
          offer.company_name?.toLowerCase().includes(query) ||
          offer.offers?.offer_name?.toLowerCase().includes(query)
        );
      } else if (this.activeButton === 'events') {
        this.FilteredEvents = this.events.filter(event => 
          event.title?.toLowerCase().includes(query) ||
          event.company_details?.company_name?.toLowerCase().includes(query)
        );
      }
    },
  },

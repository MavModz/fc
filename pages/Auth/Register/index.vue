  ...
  methods: {
    async submitForm(v) {
      ...
      if (this.$route.query.id) {
        try {
          const response = await homePost(`/claim-register-user/${this.$route.query.id}`, v);
          if (response.status) {
    ...
    async loginUser(v) {
      ...
      await homePost('/login-user', v).then((response) => {
        if (response.data.status) {
          ...
          if (this.$route.query.admin==='newadmin') {
            return this.$router.replace(`/adminProfile?newUser=true`);
          }
          if (this.$route.query.id) {
            this.$router.replace(`/${response.data.user.role.toLowerCase()}/profile?newUser=true&id=${this.$route.query.id}`);
          }
          else {
            this.$router.replace(`/${response.data.user.role.toLowerCase()}/profile?newUser=true`);
          }
        }
      });
      this.loadinga = false;

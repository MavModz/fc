...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const payload = this.update ? {} : { step: this.company.step === 2 ? 3 : undefined, update: false };
      if(this.$route.query.company_id){
        this.superadmin_step=4
      }
    
      try {
        const superadminStep = this.$route.query.company_id ? 4 : 1;
        console.log(superadminStep)
        await superadminPost(`/superadmin-company-reapply/${this.company._id}`, { services: data, ...payload ,superadmin_step:superadminStep 
        });
        ...
        console.log("this.categoryUpdateeeeeeee")
  ...
  async setup() {
    ...
    await superadminGet('/superadmin-countries_id').then(response => {
  ...
  async mounted() {
    ...
    if (this.company.step > 2||this.company.superadmin_step==3) {
      for (let i in this.company.services) {
        let c = this.company.services[i]

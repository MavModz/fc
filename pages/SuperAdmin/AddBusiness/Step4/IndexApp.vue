          ...
          <center>
            <div class="col-sm-8 mb-3 mt-5">
              <div class="text-center text-lg-start">
                <p class="h4 fw-bold lh-1 text-body-emphasis mb-3" style="">Complete Your Profile</p>
              ...
              <div id="complete-prf-onbrding" class="rounded-3 mt-5 bg-body-tertiary">
                <div class="row">
                  <div class="col-lg-6 form-group pt-4 pt-md-0 pb-3">
                    <label for="" class="mb-1">Pin Code</label>
                    <FormKit validation="required" type="text" v-model="company.pincode" class="form-control py-3"
                      name="pincode" placeholder="Enter your PIN Code" maxlength="6" @keypress="restrictToNumbers" validationLabel="PIN Code"/>
                  </div>
                  <div class="col-lg-6 form-group pt-4 pt-md-0 pb-3 relative">
                    <label for="" class="mb-1">Country of Residence</label>
                  ...
                  <div class="form-group pt-4 pt-md-0 pb-3 relative">
                    <label for="countrySelect" class="mb-1">State</label>
                  ...
                  <div class="form-group pt-4 pt-md-0 pb-3 relative">
                    <label for="countrySelect" class="mb-1">City</label>
                ...
                <div class="form-group pt-4 pt-md-0 pb-3 relative">
                  <label for="" class="mb-1">Your Address</label>
                  <FormKit validation="required" type="text" name="address" v-model="company.address"
                    class="form-control py-3" placeholder="Please enter your address" validationLabel="Address"/>
                ...
                <div class="form-group pt-4 pt-md-0 pb-3 relative">
                  <label for="" class="mb-1">Your Landmark</label>
                  <FormKit validation="required" type="text" name="landmark" v-model="company.landmark"
                    class="form-control py-3" placeholder="Please enter Landmark" validationLabel="Landmark"/>
...
export default {
  ...
  async mounted() {
    this.allCountry = (await superadminGet('/superadmin-countries')).data;
    this.initializeMap();

    if (this.company.step > 3 ||this.company.superadmin_step==4) {
      await this.getState()
      await this.getCity()
      this.setLocation()
    }
  ...
  this.loading.a = true
  v.superadmin_step=5
  await superadminPost(`/superadmin-company-reapply/${this.company._id}`, v).then(response => {
    if(this.update){
      this.$router.push({ 
        path: '/admin/business-profile'
      })
  ...
  this.loading.a = false
    ...
    async getState() {
      await superadminGet(`/states/${this.company.country}`).then(response => {
        this.allState = response.data
      })
    },
    async getCity() {
      await superadminGet(`/city/${this.company.state}`).then(response => {

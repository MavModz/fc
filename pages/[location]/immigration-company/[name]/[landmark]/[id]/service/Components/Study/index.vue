<template>
    <main>
        <div class="parents-container-listing">
            ...
            <div class="services-thin-scrollbar style-4" :class="{ 'scroll-active': isScrollActive }">
                <div class="country-cards-data">
                    ...
                    <template v-else-if="visibleAchievers?.length > 0">
                        <div v-for="(item, index) in visibleAchievers" :key="item._id"
                            class="country-card-container pr-2">
                            <!-- <div @click="navigateToDetails(item)"> -->
                                <div>
                                <div class="image-area nuxt-link-hover relative">
                                    <img :src="item?.study?.cover_image" class="card-img" />
            ...
            <div>
                <template v-if="visibleAchievers?.length == 0">
                    <Emptycontent />
                </template>
            </div>
            <!-- Load More Button -->
...
export default {
    data() {
        return {
            ...
            loading: true, // New state to track loading
        };
    },
    components: {
        Emptycontent

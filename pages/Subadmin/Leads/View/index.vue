<template>
  ...
  <div v-else>
    ...
    <DataTable class="gallery-container leadabc" ref="dt" :value="customers" paginator :rows="20" dataKey="id"> -->
      <template #header>
        <div class="gallery-media-action">
          ...
          <div class="leads-filter-design">
            ...
            <!-- <Button class="your-events-btn" icon="pi pi-external-link" label="Export Csv" @click="exportCSV($event)" /> -->
            <div class="leads-filter-right">
              ...
              <!-- <Button class="your-events-btn" icon="pi pi-angle-down" label="More Option"  /> -->
            ...
            <!-- <NuxtLink to="/subadmin/leads/add">
              <button class="your-events-btn"><span class="your-event-addsign">+</span> Add New Lead</button>
            </NuxtLink> -->
            <!-- </NuxtLink> -->
            <!-- <NuxtLink to="/subadmin/leads/csv">
              <button class="your-events-btn"><span class="your-event-addsign">+</span>Upload Csv</button>
            </NuxtLink> -->
          </div>
        </div>
      </template>
    ...
    <FormKit type="form" @submit="addFollowup">
      ...
      <div class="modal-footer-section">
        <button @click="addModal = false" type="submit" class="followup-btn-btn-cancel" data-bs-dismiss="modal"
          id="close">Go
          back</button>
        <!-- <button @click="addModal = false" type="submit" class="followup-btn-btn-cancel" data-bs-dismiss="modal" id="close">Go
          back</button> -->
        <button type="submit" class="followup-btn-btn-dle-appt">Submit</button>
      </div>
    </FormKit>
...
export default {
  ...
  data() {
    return {
      ...
      metaKey: true,
      items: [
        { label: 'Edit', command: (e) => { this.$router.push(`/subadmin/leads/update/${e.id}`) } },
        { label: 'Details', command: (e) => { this.$router.push(`/subadmin/leads/detail/${e.id}`) } },
        { label: 'Delete', command: (e) => { this.deleteLead(e.id) } },
        { label: 'Convert To Customer', command: (e) => { this.assignModal = true; this.curr_id = e.id; } },
        { label: 'Add Follow Up', command: (e) => { this.addFollowupodal = true; this.curr_id = e.id; } },
      ],
      lead_statuss: null,
      allPipeline: [],
      filterBar: false,
  ...
  methods: {
    ...
    async applyFilter(i) {
      this.loading = true;
      await adminPost(`/subadmin-all-leads`, { filter: i }).then(async (response) => {
    ...
    async init() {
      this.loading = true;
      await adminGet(`/subadmin-all-leads`, { filter: { customer_type: 'lead' } }).then(async (response) => {
        this.customers = this.getCustomers(response.data.data);
        this.loading = false;
      });
      this.loading = false;

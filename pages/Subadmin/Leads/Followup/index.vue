<template>
  ...
  <div>
    ...
    <DataTable v-model:selection="selectedCustomer" class="gallery-container leadabc follow-up-lead" ref="dt"
      ...
      <Column field="name" header="Lead Name" class="test" :filterMenuStyle="{ width: '14rem' }"
        ...
        <template #body="{ data }">
          <div class="all-leads-tabl-eye">
             ...
             <!-- @click="this.$refs.canvas.openCanvas(data)" -->
             <span
              class="leads-table-namee-wgt">{{ data.customer_id.name
              }}</span><span class="lead-date text-[#64748b]" style="display: block; margin-top: 4px">
              {{ formatDate(data.customer_id.date) }}
            </span>
          </div>
        </template>
      </Column>
      ...
      <Column field="assigned_to.name" header="assigned_to" filterField="assigned_to.name" :showFilterMatchModes="false"
      ...
      <Column class="test-followup" header="Action">
        <template #body="{ data }">
          <div class="btn-align-center split-custom-class">
            <SplitButton class="leads-customopen-drpp" :model="getItems(data)"></SplitButton>
          </div>
        </template>
      </Column>
    </DataTable>
...
export default {
  ...
  data() {
    return {
      ...
      items: [
        { label: 'Edit', command: (e) => { this.$router.push(`/subadmin/leads/update/${e.data.customer_id._id}`) } },

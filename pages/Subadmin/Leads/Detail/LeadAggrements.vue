    ...
    <div v-if="agreementStore && agreementStore.length == 0" class="card bg-white border-0 rounded-10">
        <div class="card-body">
          <div class="text-center">
            ...
            <NuxtLink to="/admin/Agreement" class="btn btn-danger py-2 px-4 text-decoration-none text-white mb-4" >Add New Agreements</NuxtLink>
          </div>
        </div>
      </div>
              ...
              <tr v-else v-for="i in allAgreements">
                <td>
                  <span class="recmf-std-vistbl">
                    <p>{{ i.agreement_id.document_title }}</p>
                  </span>
                </td>
                <td>
...
export default {
  ...
  mounted() {
    ...
    async init() {
      this.loading = true
      this.agreementStore = (await adminGet(`/view-agreement`)).data.data.map(i => ({ label: i.document_title, value: i._id }));
    ...
    async addRecommendation(v) {
      this.loading = true
      await adminPost(`/send-agreement/${v.a_id}/${this.$route.params.id}`)
      this.loading = false
      this.init()
    },
